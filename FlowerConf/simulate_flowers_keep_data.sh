...
while [  $THETA -le 90 ]; do
	echo "now simulating for theta = $THETA"
		...
		grep "^HTsim" flower_burst.inc1.id1.sim | wc -l >> 4_flower_count.burst.out
		grep -E "^(HTsim|TI)" flower_burst.inc1.id1.sim > 4_flower_$THETA.burst.out
...
while [  $THETA -le 90 ]; do
	echo "now simulating for theta = $THETA"
		...
		sed -i "21s/.*/$beam/" BurstSource.source
		cosima -v 0  BurstSource.source > /dev/null
		grep "^HTsim" flower_burst.inc1.id1.sim | wc -l >> 5_flower_count.burst.out
		grep -E "^(HTsim|TI)" flower_burst.inc1.id1.sim > 5_flower_$THETA.burst.out
		rm flower_burst.inc1.id1.sim
		let THETA=THETA+$1
done
